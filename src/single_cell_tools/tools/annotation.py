...
def rank_genes_to_excel(adata,filename,n_genes):
    """
    ...
    **Returns:**
    ...
    for k,group in enumerate(adata.uns["rank_genes_groups"]["names"].dtype.names):
        ...
        pop1 = adata.obs.loc[:,"leiden"]==group
        pop2 = adata.obs.loc[:,"leiden"]!=group
        l.loc[:,"logfoldchanges"] = [np.log(adata[pop1,adata.var.loc[:,"gene_name"]==k].X.mean()/adata[pop2,adata.var.loc[:,"gene_name"]==k].X.mean()) for k in l.loc[:,"names"]]
